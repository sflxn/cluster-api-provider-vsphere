...
func (pv *Provisioner) deleteVMESX(s *SessionContext, ctx context.Context, machine *clusterv1.Machine, moref string) error {
	var vm mo.VirtualMachine
	vmref := types.ManagedObjectReference{
		Type:  "VirtualMachine",
		Value: moref,
	}
	klog.Infof("[DEBUG] Destroying VM, retrieving runtime power state ***")
	err := s.session.RetrieveOne(ctx, vmref, []string{"name", "runtime.powerState"}, &vm)
	if err != nil {
		return err
	}
	pv.eventRecorder.Eventf(machine, corev1.EventTypeNormal, "Killing", "Killing machine %v", machine.Name)
	klog.Infof("[DEBUG] Destroying VM, checking power state ***")
	vmo := object.NewVirtualMachine(s.session.Client, vmref)
	if vm.Runtime.PowerState == types.VirtualMachinePowerStatePoweredOn {
	...
	klog.Infof("[DEBUG] Destroying VM ***")
